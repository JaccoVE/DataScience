...
# Folder Locations
f_main <- "/home/jacco/Documents/Git/DataScience/NDW/Tableau Input Files/"
...
foreach(i=1:length(f_input)) %dopar% {
  
  i <- 1

  # Load meta data intensity
  data = data.table::fread(file = paste(f_main, f_input[i], ".csv", sep="", collapse=NULL),
                                          nThread = 1)
